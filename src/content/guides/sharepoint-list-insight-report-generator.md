...
In addition to a configured SharePoint knowledge source, the agent needs SharePoint connector actions, agent flows, or equivalent tools for approved-list discovery and schema retrieval, paged item retrieval, and **Create file**. Configure **Create file** to publish only to a destination whose audience is no broader than the source list and items.

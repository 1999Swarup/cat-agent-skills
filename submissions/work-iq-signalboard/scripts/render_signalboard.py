...
SOURCE_KEYS = ("calendar", "mail", "teams", "files")
ACTIVITY_KEYS = (
    "meeting_days",
    "meetings",
    "meeting_hours",
...
def validate(doc: Any) -> dict[str, Any]:
    ...
    _enum(coverage["files"], ("complete", "unavailable"), "coverage.files")
    ...
    if coverage["files"] == "unavailable" and activity["files_touched"] != 0:
        raise ValidationError("files_touched must be 0 when Files is unavailable")
...
def source_card(key: str, coverage: dict[str, str], activity: dict[str, Any]) -> str:
    ...
    if unavailable:
        ...
        value, unit = fmt_number(activity["files_touched"]), "files touched"
        detail = "Created or edited"
...
def rhythm_html(rhythm: dict[str, list[int]], available: bool) -> str:
    ...
    for day in DAY_KEYS:
        cells = "".join(
            f'<td><span class="heat level-{level}" title="{DISPLAY[day]} '
            f'{DISPLAY[TIME_KEYS[index]]}: level {level}"></span></td>'
            for index, level in enumerate(rhythm[day])
        )
        rows.append(f"<tr><th>{DISPLAY[day]}</th>{cells}</tr>")
    headings = "".join(f"<th>{DISPLAY[key]}</th>" for key in TIME_KEYS)
    return (
        '<table class="rhythm"><thead><tr><th></th>'
...
def mail_flow_html(activity: dict[str, int], available: bool) -> str:
    ...
    return f"""
      ...
      <ul class="mail-flow">
        <li><div><span>Sent</span><b>{fmt_number(sent)}</b></div><div class="flow-track"><i style="width:{sent_share:.2f}%;background:#2563eb"></i></div></li>
        <li><div><span>Received</span><b>{fmt_number(received)}</b></div><div class="flow-track"><i style="width:{received_share:.2f}%;background:#14b8a6"></i></div></li>
      </ul>"""
...
def weekly_html(doc: dict[str, Any]) -> str:
    ...
    for source, label, key, total in series:
        ...
        cells = "".join(
            f'<li><span>W{index + 1}</span><strong>{fmt_number(value) if available else "—"}</strong>'
            f'<i style="height:{max(4, value / maximum * 48) if available else 4}px"></i></li>'
...
def observations(doc: dict[str, Any]) -> list[str]:
    ...
    if coverage["files"] != "unavailable":
        notes.append(f'{fmt_number(activity["files_touched"])} files were created or edited.')
    return notes
...
.coverage-panel{{background:var(--ink);color:white;}}.coverage-panel .section-kicker{{color:#76e4d6}}.coverage-panel h2{{color:white}}.coverage-panel>p{{margin:9px 0 0;color:#cbd4e3;font-size:12px;}}.coverage-grid{{display:grid;grid-template-columns:repeat(2,1fr);gap:10px;margin:20px 0 0;padding:0;list-style:none;}}.coverage-grid li{{position:relative;padding:15px;border:1px solid rgba(255,255,255,.13);border-radius:14px;background:rgba(255,255,255,.055);}}.coverage-grid li:after{{content:"";position:absolute;width:30px;height:5px;right:10px;top:12px;border-radius:99px;background:var(--receipt);}}.coverage-grid span{{display:block;color:#cbd4e3;font:700 9px Consolas,monospace;text-transform:uppercase;letter-spacing:.09em;}}.coverage-grid strong{{display:block;margin-top:15px;font-size:13px;}}.coverage-grid .unavailable{{opacity:.55;}}
...
@media(max-width:900px){{.hero{{grid-template-columns:1fr;}}.hero-copy{{padding:38px 32px 8px;}}.hero-art{{min-height:280px;}}.hero-art img{{left:0;width:100%;object-position:center;}}.source-grid{{grid-template-columns:repeat(2,1fr);}}.dashboard-grid,.notes{{grid-template-columns:1fr;}}.weekly-panel{{grid-column:auto;}}}}
@media(max-width:560px){{.page{{width:min(100% - 20px,1180px);}}.hero{{border-radius:22px;}}.hero-copy{{padding:28px 22px 2px;}}.source-grid{{grid-template-columns:1fr;}}.panel{{padding:20px;border-radius:20px;}}.panel-head{{align-items:start;flex-direction:column;}}.panel-head p{{text-align:left;}}.rhythm{{border-spacing:3px 8px;}}.rhythm thead th{{font-size:8px;}}.week-row{{grid-template-columns:1fr;}}.coverage-grid{{grid-template-columns:1fr 1fr;}}footer{{flex-direction:column;}}}}
@media(prefers-reduced-motion:no-preference){{.hero,.source-card,.panel{{animation:arrive .55s both ease-out;}}.source-card:nth-child(2){{animation-delay:.05s}}.source-card:nth-child(3){{animation-delay:.1s}}.source-card:nth-child(4){{animation-delay:.15s}}@keyframes arrive{{from{{opacity:0;transform:translateY(10px)}}to{{opacity:1;transform:none}}}}}}
@media print{{body{{background:white}}.page{{width:100%;padding:0}}.hero,.source-card,.panel{{box-shadow:none;break-inside:avoid}}}}
</style>
</head>
<body>
<main class="page">
  ...
  <section class="notes">
    ...
    <article class="panel coverage-panel"><div class="section-kicker">Signal check</div><h2>What made the cut</h2><p>Missing sources stay missing—never zeroed into the story.</p>{coverage_html(coverage)}</article>
  </section>
